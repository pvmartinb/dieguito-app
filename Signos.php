...
session_start();
include 'cnx.php';
function muestraEsperaSignos(){
	global $conectar;
	$sql="select count(expediente) as n from usuarios where zona_siguiente=2";
	...
	echo "Pacientes esperando para toma de signos: ".$n."<br>";	
	}
	...
	<form>
		<?php
			if (isset($_GET['inicio'])) {
				...
				$sql.="2)";
				$res=mysqli_query($conectar,$sql);
				$sql="update usuarios set zona_actual=2, zona_siguiente=5 where expediente=".$_SESSION['expediente'];
				$res=mysqli_query($conectar,$sql);
				$res=mysqli_query($conectar,"COMMIT");
				$_SESSION['ID']=$max;
				muestraEsperaSignos();
